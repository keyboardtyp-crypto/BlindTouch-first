...
export default function FingerPracticePage() {
  ...
  const checkIsUnlocked = (stepIndex: number) => {
    ...
    // Step 32 (idx: 31) -> Step 18 完了時 (highestIdx >= 18)
    if (stepIndex === 31 && highestIdx >= 18) return true;

    // Step 33 (idx: 32) -> Step 24 完了時 (highestIdx >= 24)
    if (stepIndex === 32 && highestIdx >= 24) return true;
        ...
        {gameState === "selecting" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {FINGER_STAGES.map((stage: Level, idx: number) => {
              const isUnlocked = checkIsUnlocked(idx);
              return (
                <button
                  key={stage.id}
                  disabled={!isUnlocked}
                ...
                >
                  <div className="flex justify-between items-center mb-1">
                    <p className="text-xs font-bold text-emerald-600">Step {idx + 1}</p>
                    {!isUnlocked && <span className="text-xs text-gray-400">🔒 ロック中</span>}
                  </div>
                  <h3 className="font-bold text-gray-800">{stage.title}</h3>
                  <p className="text-xs text-gray-400 mt-2 font-mono">対象キー: {stage.keys}</p>
                </button>
              );
            })}
          </div>
